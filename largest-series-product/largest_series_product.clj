...
(defn largest-product
  ...
  (cond
    ...
    (throw (ex-info (str "Invalid digit(s): " digits)))

    (> span (count digits))
    (throw (ex-info (format "span=%d is greater than digits count=%d" span (count digits))))

    (neg? span)
    (throw (ex-info (str "Negative span: " span)))
